...
#define FLOAT_TO_FIXED(x) ((uint16_t)((x)*256.0f))
#define FIXED_TO_FLOAT(x) ((float)(x) / 256.0f)

#define MINDELAY 100  // Minimal 100us antara steps untuk ESP32
...
static volatile uint32_t fixed_global_multiplier = 256;  // Initialize to 1.0 in 8.8 fixed-point
static volatile uint32_t fixed_max_multiplier = 256;     // Initialize to 1.0 in 8.8 fixed-point

static volatile uint32_t cmddelay[(NUMCMDBUF + 1)], cmd0, cmd0b;
...
void newdircommand(int laserval) {
  ...
  // Debug direction bits
  zprintf(PSTR("Direction bits: "));
...
void backforward() {
  ...
  */
  ...
  // Ensure tailok is at least 5 moves behind head and in front of tail
  int moves_behind = 0;
  while (moves_behind < 5 && tailok != head && tailok != tail) {
    tailok = prevbuff(tailok);
    moves_behind++;
  }

  // Ensure tailok is in front of tail
  while (tailok == tail) {
    tailok = nextbuff(tailok);
  }
}
...
*/
  ...
  // g0 ||
  //cf=fmax((cf-1),1);
  if (ishoming || ((x2[0] == 0) && (x2[1] == 0) && (x2[2] == 0))) {
  } else cf *= f_multiplier * f2_multiplier;
...
// static THEISR void decodecmd() {
  ...
  if (cmcmd) {
    ...
    ppve = pve;
    pve = cve;
    cve = ((cmd >> 5));  // * (fixed_max_multiplier<fixed_global_multiplier)?fixed_max_multiplier:fixed_global_multiplier) >> 8;
    // 64 >> 1 + 64 >> 2 + 64 > 3
    //   32         16          8
    //   * 73 / 64 (shr 6)
    // stepdiv * 7
    ...
    if (pwmOn && lasermode == 1) pwm_val = pwm_val0 * cmdveC;
    else pwm_val = pwm_val0;
  } else {
    // this is motion header command, contain the motor direction and the laser on/off
    //cmtail = nextbuffm(cmtail);
    //cmdb = cmddelay[cmtail];
    fixed_max_multiplier = 256;  //(cmdb >> 16) & 0xFFFF;
    cmbit = (cmd >> 1) & 255;
    dirbit = cmbit;
    cmdly = DIRDELAY;
    stepdiv3a = (cmd >> 13);
    ...
    if (lasermode == 1) TOOL1(toolWasOn ? TOOLON : !TOOLON);
    // 8 bit
...
#define MINDELAY 200
  ...
  if (cmcmd) {  // 1: move
    ...
    x_dir[3] = 0;
    if (cmbit & 2) motor_0_DIR(x_dir[0] = (cmbit & 1) ? -1 : 1);
    x_dir[2] = 0;
    if ((lasermode != 1) && (cmbit & 32)) motor_2_DIR(x_dir[2] = (cmbit & 16) ? -1 : 1);
    if (cmbit & 128) {
    }
    if (cmbit & 8) {
      x_dir[1] = (cmbit & 4) ? -1 : 1;
...
int motionloop() {
  ...
  if (!RUNNING || PAUSE) {
    // Jangan loop terlalu cepat saat pause
    delay(1);
    return 0;
  } else {
    busy = 1;
...
void waitbufferempty(bool fullspeed) {
  ...
  while (true) {
    ...
    bool bufferNotEmpty = (head != tail) || m || (cmhead != cmtail) || (endstopstatus < 0);
    //interrupts();
    ...
    if (fullspeed) {
      ...
      //noInterrupts(); // Disable interrupts
      bool commandBufferEmpty = (cmhead == cmtail);
      //interrupts(); // Re-enable interrupts

      if (commandBufferEmpty) break;  // if this pause loop, then break when
    }
    servo_loop();
    if (stopping) break;
    //if (PAUSE)break;
...
void initmotion() {
  ...
  // esp_task_wdt_init(&config);  // 10 detik timeout
  ...
  zprintf(PSTR("X - Dir: %d Step: %d\n"), mdir_pin[0], mstep_pin[0]);
  zprintf(PSTR("Y - Dir: %d Step: %d\n"), mdir_pin[1], mstep_pin[1]);
  zprintf(PSTR("Z - Dir: %d Step: %d\n"), mdir_pin[2], mstep_pin[2]);

  // Test motor pins
  for (int i = 0; i < NUMAXIS; i++) {
    if (mdir_pin[i] > -1) {
      digitalWrite(mdir_pin[i], HIGH);
      delay(100);
      digitalWrite(mdir_pin[i], LOW);

      digitalWrite(mstep_pin[i], HIGH);
      delay(100);
      digitalWrite(mstep_pin[i], LOW);
    }
  }
}
