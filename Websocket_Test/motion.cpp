...
void newdircommand(int laserval)
{
  ...
  pushcmd();
   // Calculate max_multiplier based on m->fn
  float max_multiplier = 1.0f;
  float segment_speed = sqrt(m->fn);  // m->fn holds the squared speed

  // Calculate the speed limit based on the limiting axis
  float speed_limit = 10;
  for (int i = 0; i < NUMAXIS; i++) {
    if (m->dx[i] != 0) {
      float axis_speed_limit = maxf[i] * m->dis / fabs(m->dx[i]);
      if (axis_speed_limit < speed_limit) {
        speed_limit = axis_speed_limit;
      }
    }
  }

  // Calculate max_multiplier
  if (segment_speed > speed_limit) {
    max_multiplier = speed_limit / segment_speed;
  }

  // Pack feed rate, laser value, and max_multiplier into cmd0b
  cmd0 = cmd0b| ((uint32_t)(FLOAT_TO_FIXED(max_multiplier)) << 16);
  //pushcmd();
...
*/
  ...
  // g0 || 
  //cf=fmax((cf-1),1);
  if (ishoming || ((x2[0] == 0) && (x2[1] == 0) && (x2[2] == 0)) ) {} else cf *= f_multiplier*f2_multiplier;
